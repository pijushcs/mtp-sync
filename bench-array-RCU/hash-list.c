...
/////////////////////////////////////////////////////////
alist_t *alistM;
alist_t *rcu_new_array(){
	alistM=malloc(21000*sizeof(alist_t));
	alistM[0].val=LIST_VAL_MIN;
	alistM[1].val=LIST_VAL_MAX;
	alistM[0].size=2;
	//printf("A1: DONE %d %d\n", alist[0].val, alist[1].val);
	return alistM;
}
...
// HASH LIST SIZE
/////////////////////////////////////////////////////////
int hash_list_size(hash_list_t *p_hash_list, alist_t *alist)
{
	int i;
	int size = 0;
...
// LIST CONTAINS
/////////////////////////////////////////////////////////
int rcu_array_contains(alist_t *alist, val_t val){
	int i;
...
/////////////////////////////////////////////////////////
alist_t *temp;
int rcu_array_add(alist_t **alist, val_t val){
	int i;
	alist_t *nTemp;
	...
	temp=malloc((nTemp[0].size+2)*sizeof(alist_t));
	memcpy(temp,nTemp,i*sizeof(alist_t));
	memcpy(temp+(i+1), nTemp+i, (nTemp[0].size-i)*sizeof(alist_t)); 
...
// LIST REMOVE
/////////////////////////////////////////////////////////
int rcu_array_del(alist_t **alist, val_t val){
	int i;
	alist_t *nTemp;
	RCU_WRITER_LOCK(0);	
	...
	temp=malloc((nTemp[0].size)*sizeof(alist_t));
		
	memcpy(temp,nTemp,i*sizeof(alist_t));
	memcpy(temp+i, nTemp+(i+1), (nTemp[0].size-i-1)*sizeof(alist_t));  
