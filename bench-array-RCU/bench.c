...
static void hash_list_init(hash_list_t **pp, alist_t **a_pp, int n_buckets) {
#ifdef IS_RLU
...
#else
#ifdef IS_RCU_ARRAY
	return rcu_array_contains(*(d->alist),key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
#else
#ifdef IS_RCU_ARRAY
	return rcu_array_add(d->alist,key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
#else
#ifdef IS_RCU_ARRAY
	return rcu_array_del(d->alist,key);
#else
	printf("ERROR: benchmark not defined!\n");
	abort();
#endif
...
static void *test(void *data)
{
	...
	if (d->uniq_id == 0) {
		...
		int size = hash_list_size(d->p_hash_list, *(d->alist));
...
int main(int argc, char **argv)
{
	...
	hash_list_init(&p_hash_list, &alist, n_buckets);
	...
	printf("Set size      : %d (expected: %d)\n", hash_list_size(p_hash_list, alist), size);
