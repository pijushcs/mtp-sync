...
/////////////////////////////////////////////////////////
// LIST UPDATE: Modified [6th June 2016]
/////////////////////////////////////////////////////////

int rcu_list_update(list_t *p_list, val_t val, int bucket) {
	int result;
	node_t *p_prev, *p_next;
	node_t *p_node;
	val_t v;
	...
	result = ((v != val) && p_next->p_next!=NULL);
	
	if (result) {
		node_t *p_new_node = rcu_new_node();
		p_new_node->val = val;
		RCU_ASSIGN_PTR(&(p_new_node->p_next), p_next->p_next);
	...
	if(result) rcu_free_node(p_next);
	
	return result;
}

int rlu_list_update(rlu_thread_data_t *self, list_t *p_list, val_t val) {
	int result;
	node_t *p_prev, *p_next;
	val_t v;
...
restart:
	...
	if (result) {
		...
		//((node_t *)(RLU_GET_COPY(p_next)))->val = val; p_next is pointing to the copy [changed in try-lock]
		p_next->val=val;
	}
...
int rcu_hash_list_update(hash_list_t *p_hash_list, val_t val)
{
	int hash = HASH_VALUE(p_hash_list, val);
	
	return rcu_list_update(p_hash_list->buckets[hash], val, hash);
}

int rlu_hash_list_update(rlu_thread_data_t *self, hash_list_t *p_hash_list, val_t val)
{
	int hash = HASH_VALUE(p_hash_list, val);
	
	return rlu_list_update(self, p_hash_list->buckets[hash], val);
}
